...
globalParamGet key =
  withCString key $ \keyPtr ->
  alloca $ \outPtr -> do
    ok <- z3_global_param_get keyPtr outPtr
    if toBool ok
      then do
        resPtr <- peek outPtr
        if resPtr == nullPtr
...
-- | Create an enumeration sort and return its constructors and recognizers.
mkEnumerationSort :: Context
                  -> Symbol         -- ^ sort name
                  -> [Symbol]       -- ^ element names
                  -> IO (Sort, [FuncDecl], [FuncDecl])
mkEnumerationSort c sym elems =
  withContextError c $ \cPtr ->
    h2c sym $ \symPtr ->
      let elemPtrs = map unSymbol elems in
      withArrayLen elemPtrs $ \len elemsPtr -> do
        let lenCUInt = fromIntegral len :: CUInt
        allocaArray len $ \constPtr ->
          allocaArray len $ \testerPtr -> do
            sortPtr <- checkError cPtr $
              z3_mk_enumeration_sort cPtr symPtr lenCUInt elemsPtr constPtr testerPtr
            sort <- c2h c sortPtr
            constDeclPtrs <- peekArray len constPtr
            testerDeclPtrs <- peekArray len testerPtr
            constDecls <- mapM (c2h c) constDeclPtrs
            testerDecls <- mapM (c2h c) testerDeclPtrs
            pure (sort, constDecls, testerDecls)
...
-- | Create a lambda-expression that binds the given symbols and evaluates to the provided body.
--
-- The body must reference the bound variables via de-Bruijn indices (use 'mkBound').
mkLambda :: Context -> [(Symbol, Sort)] -> AST -> IO AST
mkLambda _ [] _ = error "Z3.Base.mkLambda: empty binder list"
mkLambda ctx bindings body = marshal z3_mk_lambda ctx $ \f ->
  marshalArrayLen sorts $ \n sortsArr ->
  marshalArray symbols $ \symbolsArr ->
  h2c body $ \bodyPtr ->
    f n sortsArr symbolsArr bodyPtr
  where
    (symbols, sorts) = unzip bindings

-- | Create a lambda-expression by abstracting over existing constants (apps).
mkLambdaConst :: Context -> [App] -> AST -> IO AST
mkLambdaConst _ [] _ = error "Z3.Base.mkLambdaConst: empty binder list"
mkLambdaConst ctx apps body = marshal z3_mk_lambda_const ctx $ \f ->
  marshalArrayLen apps $ \n appsArr ->
  h2c body $ \bodyPtr ->
    f n appsArr bodyPtr
...
-- | Array read with one or more indices.
mkSelectN :: Context -> AST -> [AST] -> IO AST
mkSelectN ctx arr idxs = marshal z3_mk_select_n ctx $ \f ->
  h2c arr $ \arrPtr ->
    marshalArrayLen idxs $ \idxNum idxArr ->
      f arrPtr idxNum idxArr
...
getSortKind ctx sort = toSortKind <$> liftFun1 z3_get_sort_kind ctx sort
  where toSortKind :: Z3_sort_kind -> SortKind
        toSortKind k
          ...
          | k == z3_char_sort               = Z3_CHAR_SORT
          | k == z3_type_var_sort           = Z3_TYPE_VAR
