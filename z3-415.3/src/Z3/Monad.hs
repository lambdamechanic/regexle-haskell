...
-- | Create an enumeration sort and return constructors and recognizers.
mkEnumerationSort :: MonadZ3 z3
                  => Symbol
                  -> [Symbol]
                  -> z3 (Sort, [FuncDecl], [FuncDecl])
mkEnumerationSort = liftFun2 Base.mkEnumerationSort
...
mkLambda :: MonadZ3 z3 => [(Symbol, Sort)] -> AST -> z3 AST
mkLambda = liftFun2 Base.mkLambda

mkLambdaConst :: MonadZ3 z3 => [App] -> AST -> z3 AST
mkLambdaConst = liftFun2 Base.mkLambdaConst
...
mkSelect arr idx = mkSelectN arr [idx]

-- | Array read with one or more indices.
mkSelectN :: MonadZ3 z3 => AST -> [AST] -> z3 AST
mkSelectN = liftFun2 Base.mkSelectN
